/**
 * The PostgreSQL twin of fixture.js — the same fixture, same numbers, same
 * names, written through Prisma instead of Mongoose.
 *
 * The two exist side by side only while the migration is in flight. A ported
 * module reads Postgres and an un-ported one still reads Mongo, so both
 * databases have to hold the same dataset or the golden snapshot compares a
 * half-empty system against a full one. fixture.js is deleted along with
 * Mongoose at the end of the migration and this becomes the only fixture.
 *
 * Run:  npm run seed:demo:pg
 *
 * Three things Mongoose did implicitly and Prisma does not, all of them
 * handled explicitly below: passwords are hashed here rather than by a
 * pre-save hook; timestamps are written by hand wherever list ordering depends
 * on them; and uploaded files are rows in `files`/`attachments` rather than
 * GridFS.
 */
...
async function fixture() {
  ...
  if (env.isProd) {
    console.error(
      '[fixture:pg] Refusing to run: NODE_ENV=production. This empties every table and creates ' +
        'accounts with publicly-known demo passwords — never appropriate for a real deployment.'
    );
    process.exit(1);
  }

  console.log(`[fixture:pg] Connected to ${env.databaseUrl.replace(/:[^:@/]*@/, ':****@')}`);
  ...
  console.log('[fixture:pg] Cleared existing data');
...
fixture().catch(async (err) => {
  console.error('[fixture:pg] Failed:', err);
  await prisma.$disconnect();
  process.exit(1);
});
